...
>>> pg = paginate({'paginator_page': p, 'page_obj_page': p.page(1)})
>>> pg['pages']
[1, 2, 3, 4, 5, 6, 7, 8]
>>> pg['records']['first']
1
...
>>> p = Paginator(range(15), 2)
>>> pg = paginate({'paginator_page': p, 'page_obj_page': p.page(8)})
>>> pg['pages']
[1, 2, 3, 4, 5, 6, 7, 8]
>>> pg['records']['first']
15
...
>>> paginate({'paginator_page': p, 'page_obj_page': p.page(1)})['pages']
[1, 2, 3, 4, 5, 6, 7, 8, 9]
...
>>> p = Paginator(range(21), 2, 1)
>>> pg = paginate({'paginator_page': p, 'page_obj_page': p.page(10)})
>>> pg['pages']
...
>>> t = Template("{% load pagination_tags %}{% autopaginate var 2 %}{% autopaginate var1 3 pagevar='page1' %}{% paginate %}{% paginate 'page1' %}")
>>> s = t.render(Context({'var': range(6), 'var1': range(10), 'request': HttpRequest()}))
>>> from HTMLParser import HTMLParser
>>> import re
>>> len(re.findall(r'\<a.* class="page.*"\>.*\</a\>', s))
7
